...
sigma_y = 0.1
Sigma_sigma_us_r1 = sigma_y /sqrt(1-rho_sigma_us_r1);

% % 
% % % Volatile Regime
mu_sigma_us_r2 = mu_sigma_us_r1;
rho_sigma_us_r2 = rho_sigma_us_r1 ;
Sigma_sigma_us_r2 = Sigma_sigma_us_r1;
m_sigma_us_r2 = m_sigma_us_r1;
...
splot1(x_vec(index1),(Rm_us_vec(index1).^(freq)-1)*rate_scale); hold on;
splot2(x_vec(index1),(Rm_us_vec(index2).^(freq)-1)*rate_scale); 
xlim([x_vec(1) x_vec(end)]);
xlabel(x_lab,'interpreter','latex');
ylabel('bps','interpreter','latex');
formataxis(gca);
...
xlim([x_vec(1) x_vec(end)]);
splot2(x_vec(index1),(Rm_eu_vec(index2).^(freq)-1)*rate_scale); 
xlim([x_vec(1) x_vec(end)]);
xlabel(x_lab,'interpreter','latex');
ylabel('bps','interpreter','latex');
formataxis(gca);
