...
% US money base M_us
%res_us_t = readmatrix('LFX_rawdata.xls','EuroData','T28:T261');
%sec_us_t = readmatrix('LFX_rawdata.xls','EuroData','U28:U261');
...
%     eval(['i_' curlist{j} '_t = readmatrix(''LFX_rawdata.xls'',''' datalist{j} 'Data'',''L28:L261'')/freq/100;']);
...
savelist = '';
savelist_fx = '';  % Separate list for inv_e variables
...
%    eval(['ln_' curlist{j} '_us_t = ln_' curlist{j} '_us_t-(1:length(ln_' curlist{j} '_us_t))''*log(mean(1+pi_' curlist{j} '_t)/mean(1+pi_us_t));']);
    ...
    savelist_fx = [savelist_fx 'inv_e_' curlist{j} ' '];
end
inv_e = inv_e_eu;
eval(['save exchange_rate_data.mat ' savelist ' ' savelist_fx ';']);
...
%% Save all data for filtering
eval(['save LFX_data.mat mu_eu mu_us inv_e Ted_us Ted_eu M_us Rb_Rm Rb_Rm_eu Rb_us M_eu ois cip Chi_D_US pi_us_t pi_eu_t DW_t ' savelist ' ' savelist_fx ';']);
