%% estimate_params_tvk.m -- GENERATED VARIANT of estimate_params_4d.m
%% Time-varying penalty spread: iota_t = (DW - 1M Tbill corridor)_t + kappa,
%% applied to BOTH currencies; x = (lambda, kappa, eta). Same criterion,
%% weights and data basis as the baseline. Generated 2026-08-06 (audit/Refine
%% item #1 redo); regenerate with scratchpad/make_tvk.py after edits to the
%% parent file rather than editing this copy by hand.
...
b_iota      = [0.010, 0.160];  % TVK: bounds on the constant wedge kappa (iota_t = iota_corr_dec + kappa)
...
x0 = [ ...
    ...
    min(max(iota_ss - mean(iota_corr_dec), b_iota(1) + 1e-4), b_iota(2) - 1e-4), ...  % TVK: kappa0
...
% TVK: corridor vector and per-month binding TED (bps)
if ~exist('iota_corr_dec','var'), error('estimate_params_tvk: driver must define iota_corr_dec (T-vector, annual decimal)'); end
iota_corr_dec = iota_corr_dec(:);
ted_max_t_bps = max(TED_s_us_t(:), TED_s_eu_t(:)) * abs_sc;
fprintf('TVK corridor: mean=%.4f  min=%.4f  max=%.4f (annual decimal)\n', ...
    mean(iota_corr_dec), min(iota_corr_dec), max(iota_corr_dec));
...
slack0 = min((1 - eta0) * (iota_corr_dec + x0(2)) * 1e4 / pi_us_ss - ted_asymp_margin * ted_max_t_bps);
if slack0 < 0
    kap_min_feasible = max(ted_asymp_margin * ted_max_t_bps * pi_us_ss / (1 - eta0) / 1e4 - iota_corr_dec);
    warning('TVK: initial kappa infeasible (min slack %.2f bps). Bumping x0(kappa) to %.4f.', ...
        slack0, kap_min_feasible * 1.02);
    x0(2) = min(b_iota(2) - 1e-4, kap_min_feasible * 1.02);
    fprintf('Adjusted x0(kappa) -> %.4f\n', x0(2));
end
...
% TVK: re-center iota entries of the multi-start list into kappa space.
for ii_ms = 2:numel(x0_list)
    v_ms = x0_list{ii_ms};
    v_ms(2) = min(max(v_ms(2) - mean(iota_corr_dec), b_iota(1) + 1e-4), b_iota(2) - 1e-4);
    x0_list{ii_ms} = v_ms;
end
if ~isempty(eta_fix)
...
kappa_opt     = x_opt(2);
iota_t_opt    = iota_corr_dec + kappa_opt;   % per-month annual decimal
iota_ss_opt   = mean(iota_t_opt);            % average penalty spread (downstream schema)
if isempty(eta_fix), eta_opt = x_opt(3); else, eta_opt = eta_fix; end
varrho_opt    = 0;
asy_opt_bps   = min((1 - eta_opt) * iota_t_opt * 1e4 / pi_us_ss - ted_asymp_margin * ted_max_t_bps);  % min slack, bps

fprintf('\n====== Estimation Results ======\n');
fprintf('  lambda    = %.6f   (lambda_us = lambda_eu)\n', lambda_opt);
fprintf('  kappa     = %.6f   (box [%.3f, %.3f]; position %.2f of range)\n', ...
    kappa_opt, b_iota(1), b_iota(2), (kappa_opt - b_iota(1)) / (b_iota(2) - b_iota(1)));
fprintf('  iota_t    = corridor + kappa: mean %.4f, min %.4f, max %.4f\n', ...
    mean(iota_t_opt), min(iota_t_opt), max(iota_t_opt));
if isempty(eta_fix)
...
fprintf('  Min per-month feasibility slack = %.2f bps\n', asy_opt_bps);
...
[sigma_us_opt, sigma_eu_opt, BP_us_opt, BP_eu_opt, CIP_opt, DW_us_opt, FF_us_opt, ...
    ...
    run_filter_5d(lambda_us_opt, lambda_eu_opt, iota_t_opt, eta_opt, varrho_opt, ...
        mu_us, mu_eu, mubar_us_t, mubar_eu_t, ploss_us, ploss_eu, ...
...
save(override_file, ...
    'lambda_us_override_val', 'lambda_eu_override_val', ...
    'eta_override_val', 'iota_ss_override_val', 'kappa_override_val', ...
...
function obj = estimate_obj_2d(x, ...
    ...
    lam_us = lam;
    lam_eu = lam;
    iot    = iota_corr_dec + x(2);   % TVK: per-month iota vector (annual decimal)
    if isempty(eta_fix)
    ...
    pen = pen + bound_pen(x(2), b_iota);   % TVK: box on kappa
    ...
    slack_bps = min((1 - et) * iot * 1e4 / pi_us_ss - ted_asymp_margin * ted_max_t_bps);
    if slack_bps < 0
        pen = pen + 1e6 * (1 + (-slack_bps) + slack_bps^2);
    end
...
function [sigma_us_t, sigma_eu_t, BP_us_t, BP_eu_t, CIP_t, DW_us_t, FF_us_t, ...
    ...
    abs_sc = 12e4;
    T_f = length(mu_us);
    if isscalar(iot), iot = iot * ones(T_f, 1); end   % TVK: per-month iota
    ...
    for tt = 1:T_f
        ...
        if matching_type == 1
            ...
            [sig_us, flag_us, ted_err_us] = solve_sigma_cd(mu_us_yt, ploss_us, iota_us_loc(tt), lam_us, et, vrh, ...
                                    sig_us_guess, sig_min_us, TED_us_tgt, abs_sc, fsolve_opt);
        else
            [sig_us, flag_us, ted_err_us] = solve_sigma_leon(mu_us_yt, ploss_us, iota_us_loc(tt), lam_us, et, vrh, ...
        ...
        if matching_type == 1
            ...
            [sig_eu, flag_eu, ted_err_eu] = solve_sigma_cd(mu_eu_yt, ploss_eu, iota_eu_loc(tt), lam_eu, et, vrh, ...
                                    sig_eu_guess, sig_min_eu, TED_eu_tgt, abs_sc, fsolve_opt);
        else
            [sig_eu, flag_eu, ted_err_eu] = solve_sigma_leon(mu_eu_yt, ploss_eu, iota_eu_loc(tt), lam_eu, et, vrh, ...
        ...
        % --- Derived series ---
        BP_us_t(tt) = Echi_m(mu_us_yt, ploss_us, sig_us, iota_us_loc(tt), lam_us, et, matching_type, vrh);
        BP_eu_t(tt) = Echi_m(mu_eu_yt, ploss_eu, sig_eu, iota_eu_loc(tt), lam_eu, et, matching_type, vrh);
        [~, ~, ~, ~, DW_us_t(tt), FF_us_t(tt)] = Chi_sys(mu_us_yt, ploss_us, sig_us, iota_us_loc(tt), lam_us, et, matching_type, vrh);
